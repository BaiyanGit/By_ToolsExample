namespace _3rdBy.ByFramework.FSM
{
    ...
    public class StateMachine
    {
        ...
        public int Switch(State state, params object[] param)
        {
            if (state == null) return -1;
            //如果当前状态已经是切换的目标状态 并且该状态不可切换至自身 无需切换 返回false
            if (CurrentState == state && !state.canSwitch2Self) return -2;
            //当前状态不为空则执行状态退出事件
            CurrentState?.OnExit();
            //更新当前状态
            CurrentState = state;
        ...
        /// <param name="stateName">状态名称</param>
        /// <returns>0：切换成功； -1：状态不存在； -2：当前状态已经是切换的目标状态，并且该状态不可切换至自身</returns>
        public int Switch(string stateName, params object[] param)
        {
            //根据状态名称在列表中查询
            var target = states.Find(m => m.name == stateName);
            return Switch(target, param);
        }
        ...
        /// <typeparam name="T">状态类型</typeparam>
        /// <returns>0：切换成功； -1：状态不存在； -2：当前状态已经是切换的目标状态，并且该状态不可切换至自身</returns>
        public int Switch<T>(params object[] param) where T : State
        {
            return Switch(typeof(T).Name, param);
        }
        ...
        public bool Switch2Next(params object[] param)
        {
            if (states.Count != 0)
            {
        ...
        /// </summary>
        /// <returns>true：切换成功； false：状态机中不存在任何状态，切换失败</returns>
        public void Switch2Last(params object[] param)
        {
            if (states.Count != 0)
            {
