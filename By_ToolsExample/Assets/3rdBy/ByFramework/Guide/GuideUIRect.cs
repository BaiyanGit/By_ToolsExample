namespace _3rdBy.ByFramework.Guide
{
    ...
    /// 矩形遮罩镂空引导
    /// </summary>
    public class GuideUIRect : UIGuideBase
    {
        /// <summary>
        /// 区域范围缓存
        /// </summary>
        ...
        /// 事件渗透
        /// </summary>
        private GuideUIPenetrate _eventPenetrate;
        ...
        {
            _eventPenetrate = gameObject.AddComponent<GuideUIPenetrate>();
            _eventPenetrate.SetTargetImage(Target);
            //获取高亮区域四个顶点的世界坐标
            Target.GetWorldCorners(_corners);
            //计算高亮显示区域咋画布中的范围
            _targetOffsetX = Vector2.Distance(WorldToCanvasPos(CurrentCanvas, _corners[0]),
                WorldToCanvasPos(CurrentCanvas, _corners[3])) / 2f;
            _targetOffsetY = Vector2.Distance(WorldToCanvasPos(CurrentCanvas, _corners[0]),
                WorldToCanvasPos(CurrentCanvas, _corners[1])) / 2f;
            //计算高亮显示区域的中心
            float x = _corners[0].x + ((_corners[3].x - _corners[0].x) / 2f);
            float y = _corners[0].y + ((_corners[1].y - _corners[0].y) / 2f);
            Vector3 centerWorld = new Vector3(x, y, 0);
            Vector2 center = WorldToCanvasPos(CurrentCanvas, centerWorld);
            //设置遮罩材料中中心变量
            Vector4 centerMat = new Vector4(center.x, center.y, 0, 0);
            _material = GetComponent<Image>().material;
            _material.SetVector("_Center", centerMat);
            //计算当前偏移的初始值
            RectTransform canvasRectTransform = (CurrentCanvas.transform as RectTransform);
            if (canvasRectTransform != null)
            {
                ...
                for (int i = 0; i < _corners.Length; i++)
                {
                    if (i % 2 == 0)
                        _currentOffsetX =
                            Mathf.Max(Vector3.Distance(WorldToCanvasPos(CurrentCanvas, _corners[i]), center),
                                _currentOffsetX);
                    else
                        _currentOffsetY =
                            Mathf.Max(Vector3.Distance(WorldToCanvasPos(CurrentCanvas, _corners[i]), center),
        ...
        public override void SetTarget(RectTransform target, Canvas canvas)
        {
            Target = target;
