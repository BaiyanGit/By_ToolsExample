namespace _3rdBy.ByTools.ExcelDataTool.Editor.ExcelConvert
{
    ...
    public class ExcelConvertRequest
    {
        ...
        /// 生成Excel对应的Asset
        /// </summary>
        public void GenerateAllAsset(string path, ExcelConvertPathType pathType)
        {
            var excelPaths = GetAllFilesAtPath(path);

            for (int i = 0; i < excelPaths.Count; i++)
            {
                new ExcelExportToAsset().Generate(excelPaths[i], pathType);
            }
        }
