...
public class EventCenter : MonoBehaviour
{
    ...
    private static void OnAddListenerJudge(Type eventEnum, Delegate callBack)
    {
        ...
        // 先给字典添加事件码,委托设置为空
        _eventTable.TryAdd(eventEnum, null);

        // 判断当前事件码的委托类型和要添加的委托类型是否一致，不一致不能添加，抛出异常
        var d = _eventTable[eventEnum];
    ...
    {
        OnAddListenerJudge(eventEnum, callBack);
        _eventTable[eventEnum] = (CallBack<T, TX>)_eventTable[eventEnum] + callBack;
    }

    public static void AddListener<T, TX, TY>(Type eventEnum, CallBack<T, TX, TY> callBack)
    {
        OnAddListenerJudge(eventEnum, callBack);
        _eventTable[eventEnum] = (CallBack<T, TX, TY>)_eventTable[eventEnum] + callBack;
    }

    public static void AddListener<T, TX, TY, TZ>(Type eventEnum, CallBack<T, TX, TY, TZ> callBack)
    {
        OnAddListenerJudge(eventEnum, callBack);
        _eventTable[eventEnum] = (CallBack<T, TX, TY, TZ>)_eventTable[eventEnum] + callBack;
    }

    public static void AddListener<T, TX, TY, TZ, TW>(Type eventEnum, CallBack<T, TX, TY, TZ, TW> callBack)
    {
        OnAddListenerJudge(eventEnum, callBack);
        _eventTable[eventEnum] = (CallBack<T, TX, TY, TZ, TW>)_eventTable[eventEnum] + callBack;
    }
    ...
    {
        if (!_eventTable.TryGetValue(eventEnum, out var d)) return;
