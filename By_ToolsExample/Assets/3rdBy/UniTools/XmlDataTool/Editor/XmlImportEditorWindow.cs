...
namespace _3rdBy.UniTools.XmlDataTool.Editor
{
    using System;
    ...
    public class XmlImportEditorWindow : EditorWindow
    {
        ...
        {
            _exportFolderType = Enum.GetNames(typeof(XmlConvertFolderType));
        ...
        private void GenerateXmlAssets()
        {
            ...
            var folderType = (XmlConvertFolderType)_exportFolderIndex;
            var sw = new System.Diagnostics.Stopwatch();
            sw.Start();
            foreach (var file in _xmlFileDictionary)
            {
                new XmlToJsonGenerator().XmlGenerateToJson(file.Value, out var json, out var fileName);
                var jsonTxt = $"{{\n \"dataList\":{json} \n}}";
                XmlConvertPathSetting.SaveJsonToFile(jsonTxt, fileName, folderType);

                if (_isExportClass)
                {
                    new JsonToClassGeneratorBy().GenerateClassesFromJson(json, fileName);
                }
            }
