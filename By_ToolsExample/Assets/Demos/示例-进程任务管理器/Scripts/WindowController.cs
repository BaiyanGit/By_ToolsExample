namespace Demos.示例_快捷键切换应用.Scripts.KeyPadAPI
{
    ...
    {
        [Header("OnGUI 窗口区域，实际显示会乘以 UI 缩放")]
        [SerializeField] private Rect windowRect = new(20, 20, 1040, 640);

        [Header("是否显示 OnGUI 主界面")]
        [SerializeField] private bool showGui = true;

        [Header("UI 整体缩放")]
        [SerializeField] private float uiScale = 1.0f;

        [Header("是否只显示有主窗口句柄的进程")]
        [SerializeField] private bool onlyShowProcessesWithMainWindow = true;

        [Header("隐藏按钮是否完全隐藏窗口，否则执行最小化")]
        [SerializeField] private bool useHideInsteadOfMinimize;

        [Header("启动时是否自动刷新一次进程列表")]
        [SerializeField] private bool refreshProcessListOnStart = true;

        [Header("快捷键第一个按键索引")]
        [SerializeField] private int hotkeyIndex1 = 1;

        [Header("快捷键第二个按键索引，0 表示不设置")]
        [SerializeField] private int hotkeyIndex2 = 17;

        [Header("进程名称搜索关键字")]
        [SerializeField] private string processSearchText = string.Empty;
        ...
        private void OnGUI()
        {
            ...
            windowRect = GUI.Window(GetInstanceID(), windowRect, DrawWindow, "快捷键切换应用 - OnGUI");
            GUI.matrix = oldMatrix;
        }
        ...
        private void DrawHotkeyAndOptions()
        {
            ...
            hotkeyIndex1 = DrawKeyDropdown("hotkey1", hotkeyIndex1, ref _hotkeyDropdown1Open, ref _dropdownScroll1, 170);

            GUILayout.Label("第二个按键", GUILayout.Width(70));
            hotkeyIndex2 = DrawKeyDropdown("hotkey2", hotkeyIndex2, ref _hotkeyDropdown2Open, ref _dropdownScroll2, 170);
            ...
            uiScale = GUILayout.HorizontalSlider(uiScale, MinUiScale, MaxUiScale, GUILayout.Width(180));
            uiScale = Mathf.Clamp(uiScale, MinUiScale, MaxUiScale);
            GUILayout.Label(uiScale.ToString("0.00"), GUILayout.Width(45));
            ...
            useHideInsteadOfMinimize = GUILayout.Toggle(useHideInsteadOfMinimize, "隐藏按钮使用完全隐藏，否则最小化", GUILayout.Width(230));
        ...
        private void DrawProcessToolbar()
        {
            ...
            bool newOnlyShow = GUILayout.Toggle(
                onlyShowProcessesWithMainWindow,
                "只显示有主窗口句柄的进程",
                GUILayout.Width(180));

            if (newOnlyShow != onlyShowProcessesWithMainWindow)
            {
                onlyShowProcessesWithMainWindow = newOnlyShow;
            }

            GUILayout.Label("搜索：", GUILayout.Width(42));
            processSearchText = GUILayout.TextField(processSearchText ?? string.Empty, GUILayout.Width(180));
            ...
            {
                GUIUtility.systemCopyBuffer = processSearchText ?? string.Empty;
        ...
        private void DrawProcessRow(int order, ProcessInfo info)
        {
            ...
            if (GUILayout.Button(useHideInsteadOfMinimize ? "隐藏" : "最小化", GUILayout.Width(65)))
            {
            ...
            {
                processSearchText = info.ProcessName;
            }
        ...
        private List<int> GetRequiredKeys()
        {
            ...
            int key1 = KeyCodeName.GetKeyValueByIndex(hotkeyIndex1);
            int key2 = KeyCodeName.GetKeyValueByIndex(hotkeyIndex2);
        ...
        private void RefreshProcessList()
        {
            ...
            try
            {
                ...
                for (int i = 0; i < processArray.Length; i++)
                {
                    ...
                    if (onlyShowProcessesWithMainWindow && info.MainWindowHandle == IntPtr.Zero)
                    {
                        continue;
                    }
        ...
        private bool IsProcessVisible(ProcessInfo info)
        {
            ...
            if (string.IsNullOrEmpty(processSearchText))
            {
                return true;
            }

            return info.ProcessName != null &&
                   info.ProcessName.IndexOf(processSearchText, StringComparison.OrdinalIgnoreCase) >= 0;
        }
        ...
        private string GetHotkeyDisplayText()
        {
            ...
            string name1 = key1 > 0 ? KeyCodeName.GetKeyNameByIndex(hotkeyIndex1) : string.Empty;
            string name2 = key2 > 0 && key2 != key1 ? KeyCodeName.GetKeyNameByIndex(hotkeyIndex2) : string.Empty;
