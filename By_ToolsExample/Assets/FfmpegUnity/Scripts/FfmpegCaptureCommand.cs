...
namespace FfmpegUnity
{
    public class FfmpegCaptureCommand : FfmpegCommand
    {
        ...
        IEnumerator captureCoroutine()
        {
            ...
            {
                audioListener_.velocityUpdateMode = AudioVelocityUpdateMode.Dynamic;
            }
                    ...
                    case CaptureSource.SourceType.Audio_AudioListener:
                        {
                            ...
                            captureImp_.AudioChannels = channels;
        ...
        protected override void Update()
        {
            ...
            if (IsFinishedBuild && audioListener_ == null)
            {
                audioListener_ = FindObjectOfType<AudioListener>();
                if (audioListener_ != null)
                {
            ...
            {
                var buffer = captureAudioListener_.Read();
                if (buffer != null && buffer.Length > 0)
                {
                    audioBuffers_[captureAudioListener_.StreamId].AddRange(buffer);
                }
            }
        }
