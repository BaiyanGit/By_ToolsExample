...
namespace FfmpegUnity
{
    public class FfmpegCommand : MonoBehaviour
    {
        ...
        IEnumerator restopCoroutine()
        {
            while (!IsAlreadyStart)
            {
                yield return null;
            }
            StopFfmpeg();
            restopCoroutineStarted_ = false;
        }

        // Stop ffmpeg commands.
        public void StopFfmpeg()
        {
            if (gameObject.activeInHierarchy && !IsAlreadyStart)
            {
                if (!restopCoroutineStarted_)
                {
                    restopCoroutineStarted_ = true;
                    StartCoroutine(restopCoroutine());
                }
                return;
            }
