...
#region 数据结构
[Serializable]
public partial class ExcelTest8 : ExcelObject, IExcelData
{
...
#region 数据处理
public partial class ExcelTest8Table : ExcelTable
{
    public static ExcelTest8Table Instance;
    
    public readonly List<ExcelTest8> dataList;
    private Dictionary<int, ExcelTest8> _dataDict = new Dictionary<int, ExcelTest8>();

    /// <summary>
    /// 构造函数
    /// </summary>
    public ExcelTest8Table()
    {
        Instance = this;
    }
    ...
    public ExcelTest8 Get(int id)
    {
        _dataDict.TryGetValue(id, out ExcelTest8 value);
        if (value == null)
        {
            Debug.LogError($"配置找不到，配置表名: {nameof(ExcelTest8)}，配置id: {id}");
        }
        return value;
    }
    ...
    public Dictionary<int, ExcelTest8> GetAll()
    {
        return _dataDict;
    }
